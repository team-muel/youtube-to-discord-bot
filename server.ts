...
import { client, startBot, logEvent, getBotRuntimeStatus, evaluateBotRuntimeStatus, getBotNextCheckInSec, forceBotReconnect } from './src/backend/bot';
...
import { externalFeatureRegistry } from './src/backend/registry/externalFeatureRegistry';
import { ChannelType } from 'discord.js';
import { JwtUser, SettingsRow, AuthenticatedRequest } from './src/backend/types';
import { MAX_LOGS_DISPLAY, getSafeErrorMessage } from './src/backend/utils';
...
async function startServer() {
  ...
  app.get('/api/sources', requireAuth, async (_req: AuthenticatedRequest, res: Response) => {
    return res.status(410).json({ error: externalFeatureRegistry.sources.errorCode });
  });

  app.post('/api/sources', requireAuth, async (_req: AuthenticatedRequest, res: Response) => {
    return res.status(410).json({ error: externalFeatureRegistry.sources.errorCode });
  });

  app.delete('/api/sources/:id', requireAuth, async (_req: AuthenticatedRequest, res: Response) => {
    return res.status(410).json({ error: externalFeatureRegistry.sources.errorCode });
  });

  app.put('/api/sources/:id', requireAuth, async (_req: AuthenticatedRequest, res: Response) => {
    return res.status(410).json({ error: externalFeatureRegistry.sources.errorCode });
  });
...
interface DiscordGuild {
  ...
  app.post('/api/test-trigger', requireAuth, async (_req: AuthenticatedRequest, res: Response) => {
    return res.status(410).json({ error: externalFeatureRegistry.youtubeCrawler.errorCode });
  });
